...
`Propose Workflow` 是一套將需求整理、提案、實作、審查與同步狀態串接起來的技能流程，目的是讓規格文件可以穩定地轉換為可執行的任務與後續產出。
...
1. 準備規格需求文檔。
2. 執行 `propose` 產出流程、驗收條件與任務清單。
3. 執行 `apply` 逐步實作任務。
4. 視需求使用 `bdd-unit-test` 補上測試。
5. 執行 `propose-sync` 將完成狀態同步回原始規格文檔。
